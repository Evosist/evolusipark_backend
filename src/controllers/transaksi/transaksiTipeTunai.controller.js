            ...
            const rows = result.rows.map((item, index) => {
                return {
                    ...
                    waktu_masuk: item.tanggal_masuk
                        ? dayjs(item.tanggal_masuk).format('YYYY-MM-DD HH:mm')
                        : '-',
                    gerbang_masuk: item.pintu_masuk?.keterangan || '-',
                    jenis_kendaraan:
                        item.kendaraan?.tipe_kendaraan?.tipe_kendaraan || '-',
                    nomor_polisi: item.nomor_polisi || '-',
                    waktu_keluar: item.tanggal_keluar
                        ? dayjs(item.tanggal_keluar).format('YYYY-MM-DD HH:mm')
                        : '-',
                    pintu_keluar: item.pintu_keluar?.keterangan || '-',
                    durasi_parkir: item.interval || '-',
                    denda:
                        parseInt(item.jumlah_denda_tiket || 0) +
                        parseInt(item.jumlah_denda_stnk || 0) +
                        parseInt(item.jumlah_denda_member || 0),
                    total_pembayaran:
                        parseInt(item.biaya_parkir || 0) +
                        parseInt(item.jumlah_denda_tiket || 0) +
                        parseInt(item.jumlah_denda_stnk || 0) +
                        parseInt(item.jumlah_denda_member || 0),
                    jenis_transaksi:
                        item.jenis_pembayaran?.jenis_payment || '-',
